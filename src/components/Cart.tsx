    ...
    if(!AuthenticatedState){
        ...
        const getuserdetails = async()=>{
            ...
            if(data){
                ...
                const user_data = await supabase.from("users").select("*").eq("phone_number","+"+data?.user?.phone).maybeSingle();
        ...
        const res = await supabase.from("cart_items").select("*").eq("user_id",user_id);
        ...
        <div className="flex flex-col h-full">
          ...
          {cartItems && cartItems.length > 0 && (
            <div className="border-t border-gray-200 bg-white p-4 sm:p-6 space-y-4 sticky bottom-0">
              ...
              {/* Checkout Button */}
              <button
                className="w-full bg-gray-900 text-white font-semibold py-3.5 px-6 rounded-xl hover:bg-gray-800 transition-all duration-200 transform hover:scale-[1.02] active:scale-[0.98] shadow-lg"
              >
                Proceed to Checkout
              </button>
